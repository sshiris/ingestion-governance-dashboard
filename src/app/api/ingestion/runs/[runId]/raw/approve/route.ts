import { approveRawDoc } from "@/server/services/ingestions/ingestion-raw-service";
...
export async function POST(req: Request, context: { params: Promise<{ runId: string }> }) {
  try {
    ...
    const result = await approveRawDoc({
      runId,
      rawDocId,
    ...
    console.error("POST /api/ingestion/runs/[runId]/raw/approve failed:", err);
    return NextResponse.json(
